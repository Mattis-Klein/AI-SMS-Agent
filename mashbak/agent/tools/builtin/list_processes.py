...
class ListProcessesTool(Tool):
    ...
    def validate_args(self, args: Dict[str, Any]) -> tuple[bool, str]:
        if args:
            return False, "list_processes does not accept arguments"
        return True, ""
    ...
    async def execute(self, args: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> ToolResult:
        try:
            ...
            if result.returncode != 0:
                return ToolResult(success=False, output="", error=result.stderr or "Failed to list processes", tool_name=self.name)
